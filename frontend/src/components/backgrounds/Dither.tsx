...
export interface DitherProps {
  ...
  mouseRadius?: number
  /** r3f 渲染循环：active 用 'always'，静止/减动效用 'demand' 只按需绘制一帧。 */
...
function DitheredWaves({
  ...
  mouseRadius,
}: Required<Omit<DitherProps, 'frameloop'>>) {
  ...
  // Three.js 要求 uniforms 对象跨帧保持同一引用；后续只在 effect/useFrame 中修改值。
  ...
  // 同步非时间类 uniform：任一参数变化即写入并按需重绘一帧（覆盖 demand 模式）。
  useEffect(() => {
